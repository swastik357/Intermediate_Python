...
The `query` subcommand searches for close approaches that match given criteria:
    ...
    $ python3 main.py query --start-date 2000-01-01
    --max-diameter 0.1 --not-hazardous
...
The `interactive` subcommand loads the NEO database and spawns
an interactive command shell that can repeatedly execute `inspect`
and `query` commands without having to wait to reload the database
each time. However, it doesn't hot-reload.
...
def date_fromisoformat(date_string):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"'{date_string}' \
...
def make_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Explore past and future close \
approaches of near-Earth objects."
    )
    ...
    inspect = subparsers.add_parser('inspect', description="Inspect"
                                    "an NEO by primary designation"
                                    "or by name.")
    ...
    inspect_id.add_argument('-p', '--pdes', help="The primary"
                            "designation of the NEO to inspect (e.g. '433').")
    inspect_id.add_argument('-n', '--name', help="The IAU name"
                            "of the NEO to inspect (e.g. 'Halley').")
    ...
    filters = query.add_argument_group('Filters',
                                       ...
                                       "approaches by their attributes"
    ...
    filters.add_argument('--min-distance', dest='distance_min', type=float,
                         ...
                         "close approaches that "
                              "pass as far or farther away"
                         "from Earth as the given distance.")
    filters.add_argument('--max-distance', dest='distance_max', type=float,
                         help="In astronomical units. Only return"
                         "close approaches that "
                              "pass as near or nearer to Earth"
                         "as the given distance.")
    filters.add_argument('--min-velocity', dest='velocity_min', type=float,
                         help="In kilometers per second. Only"
                         "return close approaches "
                              "whose relative velocity to Earth at"
                         "approach is as fast or faster "
    ...
    filters.add_argument('--max-velocity', dest='velocity_max', type=float,
                         ...
                         "Only return close approaches "
                              "whose relative velocity to Earth"
                         "at approach is as slow or slower "
                              "than the given velocity.")
    filters.add_argument('--min-diameter', dest='diameter_min', type=float,
                         ...
                         "diameters as large or larger than"
                         "the given size.")
    ...
    filters.add_argument('--hazardous', dest='hazardous',
    ...
    query.add_argument('-o', '--outfile', type=pathlib.Path,
                       help="File in which to save structured results. "
                            "If omitted, results are printed"
                       "to standard output.")

    repl = subparsers.add_parser('interactive',
                                 description="Start an interactive"
                                 "command session to repeatedly"
                                 "run `interact` and `query` commands.")
    repl.add_argument('-a', '--aggressive', action='store_true',
                      help="If specified, kill the session whenever"
...
def inspect(database, pdes=None, name=None, verbose=False):
    ...
    This function fetches an NEO by designation or by name.
    If a matching NEO is
    ...
    :param database: The `NEODatabase` containing data on NEOs
    ...
    :param verbose: Whether to additionally print all of a
    matching NEO's close approaches.
    ...
    # Display information about this NEO, and optionally
    # its close approaches if verbose.
    print(neo)
    if verbose:
...
def query(database, args):
    ...
    10 entries if no limit was specified. If an output file was
    given, use the
    file's extension to infer whether the file should hold CSV or
    SON data, and
    ...
    if not args.outfile:
        ...
        if args.outfile.suffix == '.csv':
            ...
            print("Please use an output file that ends"
                  "with `.csv` or `.json`.", file=sys.stderr)


class NEOShell(cmd.Cmd):
    """Perform the `interactive` subcommand.

    This is a `cmd.Cmd` shell - a specialized tool for
    command-based REPL sessions.

    It wraps the `inspect` and `query` parsers to parse flags
    for those commands
    ...
    def __init__(self, database, inspect_parser, query_parser,
        ...
        Creating this object doesn't start the session - for that,
        use `.cmdloop()`.
        ...
        file is changed.
        :param kwargs: A dictionary of excess keyword arguments passed
        to the superclass.
        """
    ...
    def parse_arg_with(cls, arg, parser):
        ...
        :return: A `Namespace` of the arguments (produced by `parse_args`)
        or None.
        """
    ...
    def do_query(self, arg):
        ...
        This command behaves the same as the `query`
        subcommand from the command
        ...
        The number of results shown can be limited to a
        maximum number with `--limit`:
    ...
    def precmd(self, line):
        ...
        changed = [f for f in PROJECT_ROOT.glob('*.py') if
                   f.stat().st_mtime > _START]
        if changed:
            print("The following file(s) have been"
                  "modified since this interactive session began: "
                  f"{', '.join(str(f.relative_to(PROJECT_ROOT))
                     for f in changed)}.", file=sys.stderr)
            if not self.aggressive:
                print("To include these changes, please"
                      "exit and restart this interactive session.",
                      file=sys.stderr)
            else:
                print("Preemptively terminating"
                      "the session aggressively.", file=sys.stderr)
                return 'exit'
        return line
...
def main():
    ...
    # Extract data from the data files into structured
    # Python objects.
    ...
    if args.cmd == 'inspect':
        ...
        NEOShell(database, inspect_parser,
                 query_parser, aggressive=args.aggressive).cmdloop()
