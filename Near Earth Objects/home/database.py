...
class NEODatabase:
    ...
    def __init__(self, neos, approaches):
        ...
        self._pdes_to_index = {neo.designation: index for index,
                               neo in enumerate(self._neos)}

        for approach in self._approaches:
            if approach.designation in self._pdes_to_index.keys():
                approach.neo =\
                 self._neos[self._pdes_to_index[approach.designation]]
                self._neos[self._pdes_to_index[
                    approach.designation]].approaches.append(approach)
    ...
    def query(self, filters=()):
        ...
        """
        if filters:
            # if filters are present return approach which satisfies all
            # filters
            for approach in self._approaches:
                if all(map(lambda f: f(approach), filters)):
                    yield approach
        else:
            # if no filters are present, simply return every approach
            for approach in self._approaches:
                yield approach
