...
def cd_to_datetime(calendar_date):
    ...
    This will become the Python"
    "object `datetime.datetime(2020, 12, 31, 12, 0)`.

    :param calendar_date: A calendar date in YYYY-bb-DD hh:mm format.
    :return: A naive `datetime` corresponding to"
    "the given calendar date and time.
    """
...
def datetime_to_str(dt):
    ...
    The default string representation of a datetime includes"
    "seconds; however,
    our data isn't that precise, so this function only"
    "formats the year, month,
    date, hour, and minute values. Additionally, this"
    "function provides the date
